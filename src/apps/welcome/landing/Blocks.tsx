...
export function LandingHero({ brand, tagline, eyebrow, headline, sub, primaryCta, secondaryCta, rating }: HeroProps) {
  return (
    <section id="top" className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-50 via-white to-rose-50 border border-stone-200/70 px-8 py-14 sm:px-14 sm:py-20 shadow-sm">
      ...
      <div className="relative flex flex-col gap-6 max-w-4xl">
        <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-indigo-700/80">
          <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-white/70 backdrop-blur border border-indigo-100 text-[10px] font-extrabold text-indigo-700 shadow-sm">{brand.split(' ').map(w => w[0]).join('').slice(0, 2)}</span>
          <span>{eyebrow}</span>
        </div>
        <h1 className="text-4xl sm:text-6xl font-extrabold leading-[1.05] tracking-tight text-stone-900" style={{ fontFamily: 'var(--theme-font-display)' }}>
          {headline}
        </h1>
        <p className="text-base sm:text-lg text-stone-600 max-w-2xl leading-relaxed">{sub}</p>
        <div className="flex flex-wrap items-center gap-3 pt-2">
          <a href={primaryCta.href} className="inline-flex items-center gap-2 rounded-full bg-stone-900 text-white px-6 py-3 text-sm font-semibold shadow-lg shadow-stone-900/20 hover:bg-stone-800 transition-all hover:scale-[1.02] active:scale-[0.98]">
            {primaryCta.label}
            <ArrowRight className="w-4 h-4" />
          </a>
          {secondaryCta && (
            <a href={secondaryCta.href} className="inline-flex items-center gap-2 rounded-full bg-white/80 backdrop-blur border border-stone-200 text-stone-800 px-6 py-3 text-sm font-semibold hover:bg-white transition-all">
        ...
        {rating && (
          <div className="flex flex-wrap items-center gap-3 pt-4 text-xs text-stone-600">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/70 backdrop-blur border border-stone-200 px-3 py-1.5 shadow-sm">
              <span className="flex gap-0.5">
                {Array.from({ length: rating.stars }).map((_, i) => (
                  <Star key={i} className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                ))}
              </span>
              <span className="font-semibold text-stone-800">{rating.count}</span>
              <span className="text-stone-500">reviews</span>
            </span>
            <div className="flex items-center gap-1.5">
              {rating.sources.map(s => (
                <span key={s} className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-white border border-stone-200 text-[10px] font-bold text-stone-600 shadow-sm">{s.charAt(0)}</span>
              ))}
            </div>
          </div>
        )}
        <div className="pt-2 text-xs text-stone-400">{tagline}</div>
      </div>
    </section>
  );
}
...
export function LogoRow({ title, logos }: { title: string; logos: Logo[] }) {
  return (
    <section className="rounded-2xl bg-white border border-stone-200/70 px-8 py-10 shadow-sm">
      <p className="text-center text-sm text-stone-500 mb-6">{title}</p>
      <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
        {logos.map(l => (
          <div key={l.name} className="flex items-center gap-2 text-stone-400 hover:text-stone-600 transition-colors">
            <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-stone-100 text-xs font-extrabold text-stone-500">{l.monogram}</span>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
      <div className={`flex flex-col gap-4 ${flip ? 'lg:order-2' : ''}`}>
        {feature.eyebrow && (
          <span className="text-[11px] font-bold uppercase tracking-[0.18em] text-indigo-700">{feature.eyebrow}</span>
        )}
        <h3 className="text-3xl sm:text-4xl font-extrabold leading-tight tracking-tight text-stone-900" style={{ fontFamily: 'var(--theme-font-display)' }}>{feature.title}</h3>
        <p className="text-base text-stone-600 leading-relaxed">{feature.body}</p>
        {feature.bullets && (
        ...
        {feature.cta && (
          <a href={feature.cta.href} className="inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-700 hover:text-indigo-900 transition-colors mt-2 self-start">
            {feature.cta.label}
            <ArrowRight className="w-4 h-4" />
          </a>
        )}
...
export function TestimonialBlock({ testimonial }: { testimonial: Testimonial }) {
  return (
    <div className="rounded-2xl bg-white border border-stone-200 shadow-sm p-8 flex flex-col gap-5 h-full">
      <p className="text-lg sm:text-xl leading-relaxed text-stone-800 font-medium">&ldquo;{testimonial.quote}&rdquo;</p>
      <div className="mt-auto flex items-end justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500" />
          <div>
            <div className="text-sm font-bold text-stone-900">{testimonial.author}</div>
            <div className="text-xs text-stone-500">{testimonial.role} · {testimonial.company}</div>
          </div>
        </div>
        {testimonial.metric && (
          <div className="text-right">
            <div className="text-3xl font-extrabold text-stone-900 tabular-nums">{testimonial.metric.value}</div>
            <div className="text-[10px] text-stone-500 uppercase tracking-wider">{testimonial.metric.label}</div>
          </div>
        )}
      </div>
    </div>
        ...
        >
          ...
          <div>
            ...
            <div className="flex items-baseline gap-1 mt-2">
              ...
              <span className={`text-sm ${t.highlight ? 'text-indigo-100' : 'text-stone-500'}`}>{t.cadence}</span>
            </div>
            <p className={`text-xs mt-2 ${t.highlight ? 'text-indigo-100/90' : 'text-stone-500'}`}>{t.pitch}</p>
          </div>
          <ul className="flex flex-col gap-2 text-sm">
            {t.features.map(f => (
              <li key={f} className="flex items-start gap-2">
                ...
                <span className={t.highlight ? 'text-indigo-50' : 'text-stone-700'}>{f}</span>
              </li>
            ))}
          </ul>
          <button
...
export function FaqAccordion({ items }: { items: FaqItem[] }) {
  ...
  return (
    <div className="rounded-2xl bg-white border border-stone-200 shadow-sm divide-y divide-stone-100 overflow-hidden">
      {items.map((item, i) => {
        ...
        return (
          <div key={item.q}>
            ...
            >
              <span className="text-sm sm:text-base font-semibold text-stone-900">{item.q}</span>
              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full transition-all shrink-0 ${open ? 'bg-indigo-600 text-white rotate-180' : 'bg-stone-100 text-stone-600'}`}>
                {open ? <Minus className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
              </span>
            </button>
            {open && (
              <div className="px-6 pb-5 text-sm text-stone-600 leading-relaxed">{item.a}</div>
            )}
          </div>
        );
      })}
