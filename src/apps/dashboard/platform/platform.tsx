...
function Knowledge() {
  ...
  return <div className="p-7" style={{ color: 'var(--theme-text)' }}>
    ...
    <div className="mb-5 grid grid-cols-4 gap-3"><StatCard label="Documents" value={DOCUMENTS.length} hint="dans le dépôt" /><StatCard label="Chunks" value={chunksTotal} hint="segments produits" tone="accent" /><StatCard label="Vectorisés" value={vectorised} hint="index en cours" tone="warn" /><StatCard label="Interrogeables" value={interrogeable} hint="prêt pour le chat" tone="ok" /></div>
...
function Members() {
  ...
  return <div className="p-7" style={{ color: 'var(--theme-text)' }}>
    ...
    <div className="mb-5 grid grid-cols-3 gap-3"><StatCard label="Membres" value={MEMBERS.length} hint="accès nominatifs" /><StatCard label="Rôles" value={ROLE_ORDER.length} hint="viewer → owner" tone="accent" /><StatCard label="Audit" value={`${auditPct}%`} hint={auditPct === 100 ? 'changements attribués' : `${attributed}/${MEMBERS.length} attribués`} tone={auditPct === 100 ? 'ok' : 'warn'} /></div>
