...
| 3 | 2026-08-17 → 2026-08-21 | `ROI_METRIC.md` publié au même chemin. Le fichier définit **une métrique de retour** (formule explicite, source de données, fréquence de mesure) et fixe **un seuil de viabilité chiffré** (valeur plancher au-dessus de laquelle l'offre est jugée rentable). | YelenaBelova (Forecasting) · RedGuardian (Reporting) | **Oui**, 2026-08-26 | Le ROI en % dégénère (division par un coût nul, symétrique du problème de S2). Remplacé par RARC (Revenu Annuel Récurrent Confirmé), source = champ `signé:` des Master Agreements — mesure $2 520/an, avec une incertitude nommée sur le tier PaaS (×7,5 selon l'ancienneté réelle, cf. escalade ci-dessous). **Seuil de viabilité fixé par le capitaine le 2026-08-26** : 100 nouveaux clients/mois, ni plus ni moins, liste d'attente au-delà — avec un modèle de cohorte tarifaire (Août-Septembre = plancher $300/an à vie, Octobre-Décembre = cible $420/an, liste d'attente = haut $504/an) qui résout au passage l'écart nommé dans `PRICING.md`. Exemple travaillé : cohorte pleine de 100 = $30 000 − $5 000 d'affiliation ($50/filleul JaaS, taux déjà dans le corpus) = $25 000 net/mois. |
| 4 | 2026-08-24 → 2026-08-28 | Trois livrables publiés : `REPRO.md` (Taskmaster documente la procédure de rejouabilité des trois fichiers précédents — script ou checklist pas-à-pas, avec au moins un **re-run à blanc** réussi) ; `FINANCIAL_DOSSIER.md` (RedGuardian consolide coût + prix + marge + ROI en un dossier de ≤ 2 pages, lisible sans ouvrir les fichiers sources) ; `COMPLIANCE_NOTE.md` (USAgent confirme la traçabilité comptable et la conformité aux obligations déclaratives en vigueur). | Taskmaster (Repro) · RedGuardian (Reporting) · USAgent (Compliance) | **Oui**, 2026-08-26 | Publié hors fenêtre calendaire, comme S1-S3. Re-run à blanc exécuté et réussi (RARC recalculé depuis les sources = $2 520/an, identique à `ROI_METRIC.md`). `COMPLIANCE_NOTE.md` tient le périmètre strict d'USAgent (traçabilité comptable) sans empiéter sur la conformité contractuelle, désormais du ressort d'Aquaman actif — l'écart PaaS année 1/2 lui est signalé, pas tranché ici. |
...
|---|---|---|---|
| 2026-08 | 4/4 | **Oui, avec réserve.** Les quatre éléments du rock (coût, prix, marge, métrique de retour) sont tous chiffrés et vérifiables comme exigé — mais « chiffré et vérifiable » n'implique pas « décisionnel ». La marge à 100 % est chiffrée sans être informative (dénominateur nul) ; le seuil de viabilité n'a pu être fixé que par une décision directe du capitaine, hors de la méthode cout/temps envisagée initialement. | Le rock a été tenu à la lettre en étant honnête sur ses propres limites, plutôt qu'en masquant une marge dégénérée derrière un chiffre à 100 %. Deux décisions humaines non déléguées ont débloqué ce qu'aucun calcul seul ne pouvait résoudre : le seuil de viabilité (capitaine) et l'affiliation JaaS déjà connue du corpus, recoupée plutôt que réinventée. Un écart contractuel (PaaS année 1/2, facteur ×7,5 sur le RARC) reste ouvert, remonté à Summers et à Aquaman, pas tranché par ce domaine. |
