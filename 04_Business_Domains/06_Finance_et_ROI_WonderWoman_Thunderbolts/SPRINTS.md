...
|---|---|---|---|---|---|
| 1 | 2026-08-03 → 2026-08-07 | `COST_MODEL.md` publié dans `04_Business_Domains/06_Finance_et_ROI_WonderWoman_Thunderbolts/dossiers/2026-08/`. Le fichier liste **chaque poste de coût** de l'offre (libellé, valeur mensuelle en EUR, source, hypothèse, classification fixe/variable, et **date de revue + métrique de retour si le poste est récurrent**). Le document porte un **total mensuel** en EUR. | Ghost (CostOpt) · BuckyBarnes (Cashflow) · USAgent (Compliance) | Oui (partiel), 2026-08-26 | Publié à la bonne date **calendaire mais hors fenêtre de sprint** — S1 était daté 08-03→08-07, ce run réel a eu lieu le 08-26 (premier run de la chaîne B1→B2→B3, jamais exécutée avant cette date). 3 postes sur 6 (Vercel, OpenRouter, GitHub) restent non confirmés faute d'accès direct aux dashboards de facturation ; total mensuel confirmé = $50/mois seulement. Bloque S2 (calcul du prix plancher) tant que non résolu. |
...
| 2026-08-26 | **Mise à jour** : Vercel, Supabase, Render, Octopus Deploy confirmés gratuits par déclaration directe de l'utilisateur. MiniMax M3 ($50/mois, seul poste actif confirmé) résilié. **Coût complet confirmé = 0 €/mois.** Conséquence pour S2 : la formule prix plancher = coût / N dégénère à 0 sur un coût nul — S2 doit ancrer le prix plancher sur autre chose (valeur perçue, ou `ADR-AAAS-PRICING-001` déjà ratifié) plutôt que sur le coût. Seul OpenRouter (openrouter.ai, distinct du crédit AgentRouter déjà mesuré à $0 consommé) reste non confirmé. | Le coût nul n'est pas une absence de blocage — c'est un nouveau type de blocage pour S2, nommé ici. |
